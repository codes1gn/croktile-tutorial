...
Choreo makes those levels explicit in the tileflow: outer **`parallel … : block`** for the block grid, inner **`parallel … : group`** for warps, and **`foreach`** loops over K that line up with **`MATMUL_TILE_K`** and **`MATMUL_MMA_K`**.
...
On **SM86**, the synchronous MMA path (`mma.sync`-class instructions under the hood) is scoped to a **single warp** (32 threads). In Choreo tileflow, that corresponds to a **`parallel`** region annotated **`: group`**—one cooperative thread group the size of one warp, not four warps as on Hopper WGMMA.
