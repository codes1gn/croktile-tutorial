...
Chapter 10 zooms in on **`__cpp__`**, macros, and other **escape hatches** for the corners of real kernels that resist pure DSL expression — so you can **graduate** from **one-line** guards to **structured** **interop** without losing **tileflow** clarity.
...
When you reuse this pattern outside MoE, rename **`expert_offsets`** mentally to whatever table defines segment starts (batch pointers, CSR row pointers, bucket heads, and so on). The tileflow spelling stays **`view` / `from`** as long as one axis is ragged and hardware still wants fixed tiles.
